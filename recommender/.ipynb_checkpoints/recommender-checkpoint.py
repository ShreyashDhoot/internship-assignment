...
RATE_LIMIT_RETRY_DELAY_SECONDS = 10 * 60


def generate_content_with_rate_limit_retry(model, prompt, **kwargs):
    try:
        return model.generate_content(prompt, **kwargs)
    except ResourceExhausted:
        print(
            f"    [RATE LIMIT] Gemini quota exhausted. "
            f"Retrying in {RATE_LIMIT_RETRY_DELAY_SECONDS // 60} minutes..."
        )
